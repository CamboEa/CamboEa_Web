...
export function AdminIndicatorManager() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (item) =>
        item.indicatorName.toLowerCase().includes(q) ||
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/indicator', { cache: 'no-store' });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(json.error || 'ផ្ទុកឯកសារ Indicator មិនបាន');
        return;
      }
  ...
  const openEditModal = (item: IndicatorItem) => {
    setEditingItem(item);
    setEditIndicatorName(item.indicatorName);
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(json.error || 'កែប្រែ Indicator មិនបាន');
        return;
      }

      toast.success('កែប្រែ Indicator បានជោគជ័យ');
      closeEditModal();
      await loadItems();
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (id: string) => {
    const ok = window.confirm('តើអ្នកចង់លុបឯកសារ Indicator នេះមែនទេ?');
    if (!ok) return;

    const res = await fetch(`/api/admin/indicator?id=${encodeURIComponent(id)}`, {
      method: 'DELETE',
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) {
      toast.error(json.error || 'លុបឯកសារ Indicator មិនបាន');
      return;
    }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
          គ្រប់គ្រង Trading Indicator
        </h1>
        <p className="text-gray-600 dark:text-gray-400">
          គ្រប់គ្រងបញ្ជី Indicator ជាទម្រង់តារាង និងទិន្នន័យពាក់ព័ន្ធ។
        </p>
      </div>
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-3 justify-between">
          <div className="flex items-center gap-3">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
              បញ្ជី Indicator ({items.length})
            </h2>
            <Link
              href="/admin/indicator/new"
